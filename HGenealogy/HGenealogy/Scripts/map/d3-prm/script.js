...
function drawGraph() {
    ...
    for (var name in graph.data) {
        ...
        for (var depIndex in obj.depends) {
			var linkto = graph.data[depIndex];			
            var link = {
    ...
    setTimeout(function() {
		...
		// var obj = graph.data["桂順"];
		// searchid = "117";
		if(selectid != ''){
			for (var name in graph.data) {
				var obj = graph.data[name];
				if(obj.nodeid.toString() == selectid)
				{
					selectObject(obj);
					break;
				}
...
function selectObject(obj, el) {
    ...
    if (nodeRect.left < graphRect.left ||
        ...
        nodeRect.left + nodeRect.width  > graphRect.left + graphRect.width / 2 ||
        nodeRect.top  + nodeRect.height > graphRect.top  + graphRect.height / 2) {
